...
check\_pgactivity - PostgreSQL plugin for Nagios
...
check\_pgactivity is designed to monitor PostgreSQL clusters from Nagios. It
...
- **-S**, **--dbservice** SERVICE\_NAME

    The connection service name from pg\_service.conf to use.
...
- **-F**, **--format** OUTPUT\_FORMAT
    ...
    The `nagios_strict` and `json_strict` formats are equivalent to the `nagios`
    and `json` formats respectively. The only difference is that they enforce the
...
- **--status-file** PATH
    ...
    calls. Default is to save a file called `check_pgactivity.data` in the same
    ...
    locking mechanism. See `perldoc -f flock` for more information.
...
check\_pgactivity allows two different connection specifications: by service or
...
- **archive\_folder**
    ...
    segments a WAL owns. Check\_pgactivity automatically handles files with
    ...
    if your archived WALs are compressed and check\_pgactivity is unable to guess
    the original size. Here are the commands check\_pgactivity uses to guess the
...
- **archiver** (8.1+)
    ...
    Required privileges: superuser (&lt;v11), grant execute on function pg\_stat\_file(text) for v11+.
...
- **backends** (all)
    ...
    a pg\_monitor (10+) or superuser (<10) role is required to see all queries.

- **backends\_status** (8.2+)
    ...
    **insufficient privilege** appears when you are not allowed to see the statuses
    ...
    'status\_label=value' separated by a comma. Available labels are `idle`,
...
- **checksum\_errors** (12+)
    ...
    Check for data checksums error, reported in pg\_stat\_database.
...
- **backup\_label\_age** (8.1+)

    Check the age of the backup label file.

    Perfdata returns the age of the backup\_label file, -1 if not present.
...
- **bgwriter** (8.3+)
    ...
    Critical and Warning thresholds are optional. If set, they _only_ accept a
    percentage.
...
- **btree\_bloat**
    ...
    **bloat** size, not object size. If a percentage is given, the threshold will
    ...
    values are passed, check\_pgactivity will choose the largest (bloat size) value.
    ...
    "database.schema\_name.relation\_name". This enables you to filter either on a
    ...
    on PostgreSQL 10+, a user with the role pg\_monitor suffices,
    provided that you grant SELECT on the system table pg\_statistic
    to the pg\_monitor role, in each database of the cluster:
...
- **commit\_ratio** (all)
    ...
    separated 'label=value'. Available labels are **rollbacks**, **rollback\_rate**
    and **rollback\_ratio**, which will be compared to the number of rollbacks, the
    rollback rate and the rollback ratio of each database. Warning or critical will
    be raised if the reported value is greater than **rollbacks**, **rollback\_rate**
    or **rollback\_ratio**.
...
- **database\_size** (8.1+)
    ...
    **Check the variation** of database sizes, and **return the size** of every
    ...
    absolute `size` and/or the `delta` size. Eg.:

        -w 'size=500GB' -c 'size=600GB'
        -w 'delta=1%' -c 'delta=10%'
        -w 'size=500GB,delta=1%' -c 'size=600GB,delta=10GB'

    The `size` label accepts either a raw number or a size and checks the total database size.
    The `delta` label accepts either a raw number, a percentage, or a size.
...
- **extensions\_versions** (9.1+)
    ...
    version available on the instance (according to pg\_available\_extensions).

    Typically, it is used to detect forgotten extension upgrades after package
    upgrades or a pg\_upgrade.
    ...
    This service supports multiple `--exclude` argument to exclude one or
    more extensions from the check. To ignore an extension only in a particular database,
    use  'dbname/extension\_name' syntax.
...
- **is\_master** (all)
    ...
    as "in production" by pg\_controldata.
...
- **invalid\_indexes** (8.2+)
    ...
    "database.schema\_name.index\_name". This enables you to filter either on a
...
- **last\_analyze** (8.2+)
    ...
    **NOTE**: this service does not raise alerts if the database had strictly
...
- **longest\_query** (all)
    ...
    Above 9.0, it also supports `--exclude REGEX` to filter out application\_name.

    You can use multiple `--exclude REGEX` parameters.

    Required privileges: an unprivileged role only checks its own queries;
    a pg\_monitor (10+) or superuser (<10) role is required to check all queries.

- **max\_freeze\_age** (all)
    ...
    thresholds are computed based on the "autovacuum\_freeze\_max\_age" parameter.
...
- **minor\_version** (all)
    ...
    website if check\_pgactivity has access to it, or must be given as a parameter.
...
- **oldest\_2pc** (8.1+)
    ...
    Check the oldest _two-phase commit transaction_ (aka. prepared transaction) in
    the cluster.
...
- **oldest\_idlexact** (8.3+)

    Check the oldest _idle_ transaction.
    ...
    filter out pg\_dump and pg\_dumpall, set this to 'pg\_dump,pg\_dumpall'.
...
- **oldest\_xmin** (8.4+)
    ...
    Check the xmin _horizon_ from distinct sources of xmin retention.
    ...
    replication slots and walsender. For versions prior to 9.4, only `2pc` source
    of xmin retention is checked.

    Using `--detailed`, Perfdata contains the oldest xmin and maximum age for the
    following source of xmin retention: `query` (a running query), `active_xact`
    (an opened transaction currently executing a query), `idle_xact` (an opened
    transaction being idle), `2pc` (a pending prepared transaction), `repslot` (a
    replication slot) and `walwender` (a WAL sender replication process), for each
    connectable database.  If a source doesn't retain any transaction for a
    database, NaN is returned.  For versions prior to 9.4, only `2pc` source of
    xmin retention is available, so other sources won't appear in the perfdata.
    Note that xmin retention from walsender is only set if `hot_standby_feedback`
    ...
    Required privileges: a pg\_read\_all\_stats (10+) or superuser (<10) role is
    required to check pg\_stat\_replication.  2PC, pg\_stat\_activity, and replication
...
- **pg\_dump\_backup**
    ...
    For compatibility with pg\_back (https://github.com/orgrim/pg\_back),
    you should use:
...
- **pga\_version**
    ...
    Check if this script is running the given version of check\_pgactivity.
...
- **pgdata\_permission** (8.2+)
    ...
    Required privileges:
     <11:superuser
     v11: user with pg\_monitor or pg\_read\_all\_setting
...
- **replication\_slots** (9.4+)
    ...
    files in pg\_replslot for each logical replication slot. Since v13, if
    `max_slot_wal_keep_size` is greater or equal to 0, perfdata reports the size
    of WAL to produce before each slot becomes `unreserved` or `lost`. Note that
    this size can become negative if the WAL status for the limited time where the
    slot becomes `unreserved`. It is set to zero as soon as the last checkpoint
    finished and the status becomes `lost`.
    ...
    of kept wal files, number of spilled files in pg\_replslot for each logical slot
    and remaining bytes before a slot becomes `unreserved` or `lost`.

    Moreover, with v13 and after, the service raises a warning alert if a slot
    becomes `unreserved`. It raises a critical alert if the slot becomes `lost`.

    Required privileges:
     v9.4: unprivileged role, or superuser to monitor spilled files for logical replication
     v11+: unprivileged user with GRANT EXECUTE on function pg\_ls\_dir(text)
...
- **streaming\_delta** (9.1+)
    ...
    provided values must be of the form "APPLICATION\_NAME IP".
    ...
    matching a regular expression on application\_name or IP address fields.
...
- **table\_bloat**
    ...
    **bloat** size, not object size. If a percentage is given, the threshold will
    ...
    If multiple threshold values are passed, check\_pgactivity will choose the
...
- **temp\_files** (8.1+)
    ...
    by tablespace (see GUC temp\_tablespaces).
    ...
    on functions pg\_ls\_dir(text), pg\_read\_file(text), pg\_stat\_file(text, boolean);
    the same restrictions than on v10 will still apply
     v12+: a role with pg\_monitor privilege.
...
- **uptime** (8.1+)
    ...
    checkpointer, writer or startup). This needs pg\_monitor access to read
    pg\_stat\_activity.
    ...
    Required privileges: pg\_monitor on PG10+; otherwise unprivileged role.
...
- **wal\_files** (8.1+)
    ...
    Required privileges:
     <10:superuser (<10)
     v10:unprivileged user with pg\_monitor
     v11+ :unprivileged user with pg\_monitor, or with grant EXECUTE on function
    pg\_ls\_waldir

## EXAMPLES

- Execute service "last\_vacuum" on host "host=localhost port=5432":

        check_pgactivity -h localhost -p 5432 -s last_vacuum -w 30m -c 1h30m

- Execute service "hot\_standby\_delta" between hosts "service=pg92" and "service=pg92s":

        check_pgactivity --dbservice pg92,pg92s --service hot_standby_delta -w 32MB -c 160MB

- Execute service "streaming\_delta" on host "service=pg92" to check its slave "stby1" with the IP address "192.168.1.11":

        check_pgactivity --dbservice pg92 --slave "stby1 192.168.1.11" --service streaming_delta -w 32MB -c 160MB

- Execute service "hit\_ratio" on host "slave" port "5433, excluding database matching the regexps "idelone" and "(?i:sleep)":

        check_pgactivity -p 5433 -h slave --service hit_ratio --dbexclude idelone --dbexclude "(?i:sleep)" -w 90% -c 80%

- Execute service "hit\_ratio" on host "slave" port "5433, only for databases matching the regexp "importantone":

        check_pgactivity -p 5433 -h slave --service hit_ratio --dbinclude importantone -w 90% -c 80%

# VERSION

check\_pgactivity version 2.6, released on Fri Jul 08 2022.
